...
import { DrawingElement, SceneData } from "src/coreRenderer/basicTypes";
import { FreeDrawing, FreeDrawingType } from "src/coreRenderer/drawingElementsTypes";
...
function createDrawingCvs(ele: DrawingElement, targetCvs: HTMLCanvasElement) {
  switch (ele.type) {
    case FreeDrawingType.freeDraw:
      ...
      const outlinePoints = getStroke(points.map(pt => {
        const ptObj = new Point(pt.x, pt.y).translate(new Vector(freeDrawing.position.x, freeDrawing.position.y)).rotate(freeDrawing.rotation);
