...
function CursorCore({
  ...
  useEffect(() => {
    if (isActive) {
      ...
      scaleBySize(cursorOuterRef.current, outSize, options.outerScale);
    } else {
      scaleBySize(cursorInnerRef.current, options.innerSize, 1);
      scaleBySize(cursorOuterRef.current, outSize, 1);
    }
  }, [
    options.innerSize,
    options.innerScale,
  ...
  useEffect(() => {
    if (isActiveClickable) {
      ...
      scaleBySize(cursorOuterRef.current, outSize, options.outerScale * 1.4);
    }
  }, [
    options.innerSize,
    options.innerScale,
